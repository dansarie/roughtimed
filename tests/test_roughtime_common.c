...
START_TEST(test_verify_signature) {
  ...
  set_fail_EVP_PKEY_new_raw_public_key(1, 1, NULL);
  set_fail_EVP_MD_CTX_new(1, 1, NULL);
  set_fail_EVP_DigestVerifyInit(1, 1, 0);
  set_fail_EVP_DigestVerify(1, 1, 2);
  for (int i = 0; i < 4; i++) {
    res = verify_signature(
        g_srep_message,
        g_srep_message_len,
...
START_TEST(test_sign) {
  ...
  /* Test with simulated internal errors. */
  set_fail_EVP_PKEY_new_raw_private_key(1, 1, NULL);
  set_fail_EVP_MD_CTX_new(1, 1, NULL);
  set_fail_EVP_PKEY_CTX_new(1, 1, NULL);
  set_fail_EVP_DigestSignInit(1, 1, 0);
  set_fail_EVP_DigestSign(1, 1, 0);
  for (int i = 0; i < 5; i++) {
    res = sign(
        g_srep_message,
        g_srep_message_len,
  ...
  set_fail_EVP_DigestSign(1, 1, 1);
  set_val_EVP_DigestSign_siglen(65);
  res = sign(
      g_srep_message,
      g_srep_message_len,
      CERTIFICATE_CONTEXT,
...
START_TEST(test_priv_to_publ) {
  ...
  set_fail_EVP_PKEY_get_raw_public_key(1, 1, 0);
  set_val_EVP_PKEY_get_raw_public_key_len(32);
  res = priv_to_publ(g_private_key, pubkey);
  ck_assert(res == ROUGHTIME_INTERNAL_ERROR);
  ck_assert_mem_eq(pubkey, g_zero_sig, 32);
  rtfun_reset();
  ...
  set_fail_EVP_PKEY_get_raw_public_key(1, 1, 1);
  set_val_EVP_PKEY_get_raw_public_key_len(33);
  res = priv_to_publ(g_private_key, pubkey);
  ck_assert(res == ROUGHTIME_INTERNAL_ERROR);
  ck_assert_mem_eq(pubkey, g_zero_sig, 32);
  rtfun_reset();
...
START_TEST(test_from_base64) {
  ...
  set_fail_EVP_DecodeBlock(1, 1, 101);
  memset(outbuf, 0xff, 100);
  len_out = 100;
...
START_TEST(test_test_cert) {
   ...
   */
   ...
   for (int i = 0; i < 2; i++) {
     set_fail_parse_roughtime_header(i + 1, i + 1, ROUGHTIME_INTERNAL_ERROR);
     res = test_cert(g_public_key, certdata, false);
     ck_assert(res == ROUGHTIME_INTERNAL_ERROR);
     rtfun_reset();

     set_fail_parse_roughtime_header(i + 1, i + 1, ROUGHTIME_SUCCESS);
     roughtime_header_t failhdr = {0};
     set_val_parse_roughtime_header_header(failhdr);
     res = test_cert(g_public_key, certdata, false);
     ck_assert(res == ROUGHTIME_FORMAT_ERROR);
     rtfun_reset();
   }

   set_fail_malloc(1, 1, NULL);
   res = test_cert(g_public_key, certdata, false);
   ck_assert(res == ROUGHTIME_MEMORY_ERROR);
   rtfun_reset();

   set_fail_verify_signature(1, 1, ROUGHTIME_INTERNAL_ERROR);
   res = test_cert(g_public_key, certdata, false);
   ck_assert(res == ROUGHTIME_INTERNAL_ERROR);
   rtfun_reset();

   for (int i = 0; i < 5; i++) {
     set_fail_get_header_tag(i + 1, i + 1, ROUGHTIME_NOT_FOUND);
     res = test_cert(g_public_key, certdata, false);
     ck_assert(res == ROUGHTIME_NOT_FOUND);
     rtfun_reset();
   }
   ...
   for (int i = 0; i < 4; i++) {
     set_fail_get_header_tag(i + 2, i + 2, ROUGHTIME_SUCCESS);
     set_val_get_header_tag_length(68);
     res = test_cert(g_public_key, certdata, false);
     ck_assert(res == ROUGHTIME_FORMAT_ERROR);
     rtfun_reset();
   }
